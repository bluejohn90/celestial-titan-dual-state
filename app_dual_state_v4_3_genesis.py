# ===========================================================
# 💎 Celestial Titan Dual-State Cloud Genesis v4.3
# 🌌 GA Pick-3 & FL Pick-4 | Priority Highlight Edition
# ===========================================================

import streamlit as st, json, os, random, datetime, pandas as pd, matplotlib.pyplot as plt
...
# ===========================================================
# ⚡ Forecast Generator with Priority Highlight
# ===========================================================
def generate_forecast(game, sets=5):
    ...
    # mark highest confidence as Titan Priority
    priority = max(out, key=lambda x:x["confidence"])
    for f in out:
...
.priority {
  background:linear-gradient(90deg,#ffcc00,#fff700);
  color:black;
  padding:6px 10px;
  border-radius:6px;
  font-weight:bold;
...
col2.metric("🌙 Lunar Phase", f"{phase_name} ({phase_percent}%)")
col3.metric("⚡ Energy Field", f"{energy_level}%")
...
st.divider()
commentary=[
    "Energy harmonics stable — precision field locked.",
    "Priority sequences aligned — watch Titan highlight sets.",
    "Forecast resonance at peak — golden pattern emerging.",
    "Data field synchronized — cosmic variance minimal."
]
...
st.markdown("<div style='text-align:center;color:#9ef0ff;'>☁️ Titan Cloud Genesis v4.3 — Priority Highlight Edition 💎</div>",unsafe_allow_html=True)
